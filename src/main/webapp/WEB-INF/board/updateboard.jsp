...
<!-- 	<input type="button" onclick="showHTML();" value="본	문 내용 가져오기" /> -->
...
<!-- 		썸네일 이미지 등록: <input type="file" name="pthumbnailimg" id="pthumbnailimg"> -->
		...
		<input type="text" name="b_target" value="${board.b_target }"  placeholder="대상 입력">
...
function imgPreView(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        // 이미지 파일이 없을 때 미리보기 이미지 초기화
        const existingImg = document.getElementById('preview' + len);
        if (existingImg) {
            existingImg.src = "";
        }
    }
    ...
    // 프레임 내의 body에 이미지 추가
    const iframe = frames['se2_iframe'];
    if (iframe && iframe.document && iframe.document.body) {
        iframe.document.body.append($img);  // 프레임의 body에 이미지 추가
    } else {
        console.error("프레임 'se2_iframe'을 찾을 수 없거나 아직 로드되지 않았습니다.");
    }
