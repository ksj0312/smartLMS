...
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
    ...
    $init: function(b) {
        ...
        this.sBlankPageURL_EmulateIE7 = "smart_editor2_inputarea.html";
            ...
            setTimeout(jindo.$Fn(function() {
                ...
                if (this.zwspEnd) {
                    if (typeof(this.zwspEnd.nodeValue) == "unknown") {
                        ...
                        if (this.zwspEnd.nodeValue) {
                            this.zwspEnd.nodeValue = this.zwspEnd.nodeValue.replace(h, "")
                        }
                ...
                try {
                    ...
                    if (typeof(JEagleEyeClient) != "undefined") {
                        ...
                        JEagleEyeClient.sendError(l, j, g)
                    }
                }
                k.moveToStringBookmark(this._sBM);
                k.collapseToEnd();
                k.select();
                k.removeStringBookmark(this._sBM)
            }, this).bind(), 0)
        }
    },
    _hasConversionTarget: function(b) {
        var a = false,
            c = new RegExp("<(" + this.aConversionTarget.join("|") + ")[^>]*>", "i");
        if (b && c.test(b)) {
            a = true
        }
        return a
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
    ...
    _addFontInMarkup: function() {
        for (var c = 0, b, d, a, e;
            (b = this.aelFontInMarkup[c]); c++) {
            if (b.firstChild) {
                d = this._getFontFamilyFromLI(b).replace(this._rxQuote, "").replace(this._rxComma, ",");
